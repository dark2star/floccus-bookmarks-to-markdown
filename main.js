...
var fbmPlugin = class extends import_obsidian.Plugin {
  ...
  async processXBELFileData() {
    ...
    try {
      let xbelData = "";
      if ((xbelFolderPath == null ? void 0 : xbelFolderPath.startsWith("http://")) || (xbelFolderPath == null ? void 0 : xbelFolderPath.startsWith("https://"))) {
        const { path: path2, username, password } = this.parseUrl(`${xbelFolderPath}/${xbelFileName}`);
        const res = await this.getWebDavFile(path2, username, password);
        if (res !== null) {
  ...
  // 获取已经保存的书签列表
  getExistBookMark(dirPath, jsonName2) {
    const files = [];
    ...
    if (jsonName2 && (jsonName2 == null ? void 0 : jsonName2.length) > 0) {
      try {
        const data = fs.readFileSync(`${dirPath}/${jsonName2}`, "utf8");
  ...
  saveContent2json(filePath, content) {
  ...
  async processBkLinks(nameSiteMapping, interval = 2e3) {
  ...
  async getWebDavFile(url, username, password) {
    let auth = "";
  ...
  parseUrl(url) {
    ...
    if (match && match.groups) {
      const { protocol, username, password, url: url2 } = match.groups;
      const path2 = `${protocol}://${url2}`;
      return {
        path: path2,
        username,
        password
...
var FBMSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Update interval (in minutes)").setDesc("Specify the interval for automatic updates. Automatic update bookmarks must be on.").addText(
    ...
    new import_obsidian.Setting(containerEl).setName("Html to Markdown API URL").setDesc("The HTML to Markdown API.").addText(
      (text) => text.setValue(this.plugin.settings.html2mdApi).onChange(async (value) => {
        this.plugin.settings.html2mdApi = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Resync bookmarks").setDesc(
